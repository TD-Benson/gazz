...
function theme_woocommerce_layoutoptions() {
	...
	foreach ($layouts as $key => $value) {
		...
		$section->option_add(new CoreOption($key.'_custom_content' , __('Category HTML section', THEME_SLUG), 'text-multiline', __('Any HTML put here will be included in it\'s own block above the content.', THEME_SLUG)));
	}
}
...
function theme_woocommerce_cart($fragments=null) {
	...
	//echo '<img src="', THEME_URI, '/images/icon-menu-sub.png"><span class="text">';
	echo '<i class="icon-caret-down"></i> <span class="text">';
	echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, THEME_SLUG), $woocommerce->cart->cart_contents_count);
	echo ' - ';
	echo $woocommerce->cart->get_cart_total();
	//echo '</span><img src="', THEME_URI, '/images/woocommerce/icon-cart.png">';
	echo '</span> <i class="icon-shopping-cart"></i>';


	// Output dropdown cart
	echo '<div class="theme-woocommerce-cart-dropdown">';
	...
	if (sizeof($woocommerce->cart->cart_contents) > 0) {
		...
		echo '<div class="buttons">
			  <a href="' . $woocommerce->cart->get_cart_url() . '" class="button small">' . __('View Cart &rarr;', THEME_SLUG).'</a>
			  <a href="' . $woocommerce->cart->get_checkout_url() . '" class="button small">' . __('Checkout &rarr;', THEME_SLUG).'</a>
			  </div>';
	}
...
function woocommerce_output_related_products() {
	...
	woocommerce_related_products($columns,6); // Display related products columns
}
...
function woocommerce_output_upsells() {
	$columns = get_option('core_woocommerce_related_item_count');
	woocommerce_upsell_display(6,6); // Display up sell products columns
}

// Redefine woocommerce_output_cross_sells()
function woocommerce_output_cross_sells() {
	$columns = get_option('core_woocommerce_related_item_count');
	woocommerce_cross_sell_display($columns,6); // Display up sell products columns
}
